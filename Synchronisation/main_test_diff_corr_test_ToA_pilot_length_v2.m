...
bits = randi(2,bits_per_symbol*2*990,1); 
...
n_tild_final=zeros(length(size_pilot),length(EbN0));

NBRE_EXP=5;

%delta_f_tild=zeros(NBRE_EXP,length(EbN0));
n_tild=zeros(NBRE_EXP,length(EbN0));

symb_tx_save=symb_tx;

for n=1:length(size_pilot)
    
    for p=1:NBRE_EXP
        %SIZE_PILOT=4*bits_per_symbol; %nbre of symbols in pilot
        LENGTH_FRAME=1000-size_pilot(n);
        %% INSERTING PILOTS
